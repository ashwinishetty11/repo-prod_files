CREATE OR REPLACE TABLE sdr_cloud_signup_spiff.sdr_assisted_signups AS
...
select s.org_id
    ...
    ,a.is_csdr
    ,row_number() over (
        partition by org_id order by activitydate
    ) as activity_rnk
FROM signup_leads s
left join activity a 
	ON coalesce(a.lead_id, a.contact_id) = s.lead_or_contact_id
where ((date_diff(date(signed_up_at), date(activitydate), day) <= 30 
		and signed_up_at > activitydate and lower(a.subject) like '%email%')
      or (date_diff(date(signed_up_at), date(activitydate), day) <= 15 
      	and signed_up_at > activitydate and a.subject like '%[Call]%'))
),
...
sdr_assisted_signups AS (
    ...
    FROM (
        SELECT *
            ,row_number() over (
                partition by org_id order by signed_up_at 
            ) as rnk
        from signup_leads
    ) a
    LEFT JOIN (
...
select a.*
    ...
    then 1 else 0 end as is_signup_current_quarter
    ,case when ownerid in ("0053a00000PBIGHAA5", "0053a00000PPTDiAAP", "0053a00000PB4u3AAD", "0053a00000PnCfMAAV","0053a00000PnScvAAF","0053a00000PnSbiAAF","0053a00000PnW6eAAF") then "Jacquelyn Yeow"
    else manager_name end as manager_name_adj
