...
DATASET = "dwh_web"
DIM_DAILY_TBLS = ["fct_inquiry_daily"]
...
with DAG(DAG_ID, schedule_interval="0 8 * * *", default_args=default_dag_args) as dag:
    ...
    for c_tble in TBLS_NO_PARTITION:
        operator[c_tble] = BigQueryOperator(
            task_id=c_tble,
            sql='/'.join(['sql', ('.'.join([c_tble, 'sql']))]),
            write_disposition="WRITE_TRUNCATE",
            use_legacy_sql=False
            )
    ...
    operator['dim_visitor'] >> operator['fct_page_visit_daily']
    operator['fct_page_visit_daily'] >> operator['rpt_web_dashboard']
    operator['lookup_sfdc_lead_to_segment'] >> operator['rpt_web_dashboard']
