...
with pages as (
  ...
  where date(visit_start_time) >= date(date_trunc(date_add(current_date(), INTERVAL -1 quarter), quarter))
     and domain in (  'www.confluent.io','docs.confluent.io','kafka-tutorials.confluent.io', 'developer.confluent.io')
)
...
,  sfdc_inquiries_campaign_type as (
  ...
  /*
    select s_id session_id
        , anonymous_id
        , form_id
        , case when form_id in (2835, 6067) then 'Confluent Cloud Sign-Up'
