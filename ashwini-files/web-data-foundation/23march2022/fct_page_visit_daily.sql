...
,  pages_docs_io as(
 ...
 , pages_dev_io as(
    (select id page_visit_id
      ...
      from  confluentdev_segment_prod.pages
      where DATE(timestamp) >= '2021-10-01' --= DATE('{{ next_execution_date.isoformat() }}')
    )
    UNION ALL
    (
  ...
  --      , row_number() over (partition by anonymous_id,s_id order by timestamp desc) rnk2_isexit
      ...
      from  kafka_tutorials_prod.pages
      where DATE(timestamp) >= '2021-10-01' --= DATE('{{ next_execution_date.isoformat() }}')
    )
)
...
select  p.session_id
        ...
        , p.visit_end_time
        , p.page_visit_id
        , case  when split(p.url_cleaned,'/')[safe_offset(2)] like '?%' then '_homepage'
                when split(p.url_cleaned,'/')[safe_offset(2)] is null then '_homepage'
                when split(p.url_cleaned,'/')[safe_offset(2)] = '' then '_homepage'
        ...
        , case  when split(p.url_cleaned,'/')[safe_offset(3)] like '?%' then '_homepage'
                when split(p.url_cleaned,'/')[safe_offset(3)] is null then '_homepage'
                when split(p.url_cleaned,'/')[safe_offset(3)] = '' then '_homepage'
                else concat('/',split(p.url_cleaned,'/')[safe_offset(3)])
            end as url_section_2

        , case  when split(p.url_cleaned,'/')[safe_offset(4)] like '?%' then '_homepage'
                when split(p.url_cleaned,'/')[safe_offset(4)] is null then '_homepage'
                when split(p.url_cleaned,'/')[safe_offset(4)] = '' then '_homepage'
