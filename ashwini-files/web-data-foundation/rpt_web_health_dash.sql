create or replace table `data-sandbox-123.Workspace_Ashwini.rpt_web_health_dash` as
with pages as (
  select session_id
          , visitor_id
          , url
          , url_cleaned
          , visit_start_time
          , visit_end_time --choose what columns from pages you need here
  --        , domain         --note - added newly 
  --        , channel_detail --note - added newly 
  --        , channel_agg    --note - added newly 
    
  --        , region --note - TBD
  --        , flg_new_vs_returned - TBD --monthly visit -
    --      , flg_new_visitor - TBD

          , url_section_1 --as web_section
          , url_section_2
          , url_section_3
     --     , rnk_page_visit
  ...
  where visit_start_time >= '2021-01-01'
   -- and visit_start_time < '2022-02-01'
      and hostname in (  'www.confluent.io','docs.confluent.io','kafka-tutorials.confluent.io', 'developer.confluent.io') -- to check - to add ashwini
)
 , seg_with_lead_data as  ( 
        select date(date_trunc(visit_start_time,month)) visit_month,  
                        t1.*, t2.lead_or_contact_id
        from pages t1 --lod - s_id, anonID, visit_start_time
  ...
  , sfdc_lead_data as (
    select  distinct --LoD - lead_or_contact_id, tactic, campaign_type,lead_or_contact_persona, createddate  
      lead_or_contact_id
      , tactic
      , campaign_type
...
, lead_metrics_pg as (
 ...
 -- , t1.lead_or_contact_id 
  ...
  order by lead_or_contact_id
)
--bring the level up after this point - to LoD 
...
,  sfdc_inquiries_campaign_type as (
  ...
  , inquiry_metrics as (
    select t1.*, t2.* except(lead_or_contact_id)
      ...
      , case when campaign_type = 'Confluent Cloud Platform' then 1 else 0 end flg_cloud_platform
    from seg_with_lead_data t1
...
FROM seg_with_lead_data p
  left join lead_metrics_pg t1 on p.url = t1.url and p.session_id = t1.session_id and p.visit_start_time = t1.visit_start_time
                                                and p.lead_or_contact_id = t1.lead_or_contact_id*/

  select date(date_trunc(t1.visit_start_time, week)) visit_start_time_week ,t1.*  
