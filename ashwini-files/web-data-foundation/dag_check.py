...
DATASET = "dwh_web"
DIM_DAILY_TBLS = ["dim_sessions_daily", "fct_page_visit_daily"]

TBLS_NO_PARTITION = ["dim_visitor", "lookup_sfdc_lead_to_ga", "lookup_sfdc_lead_to_segment"]
...
with DAG(DAG_ID, schedule_interval="0 8 * * *", default_args=default_dag_args) as dag:
    ...
    for d_daily in DIM_DAILY_TBLS:
        dim_daily_tbl_tasks.append(
            BigQueryOperator(
                task_id=d_daily,
                sql=  '/'.join(['sql', ('.'.join([d_daily, 'sql']))]), # f"sql/{d_daily}.sql"
                destination_dataset_table=f"{config.PROJECT_ID}.{DATASET}.{d_daily}$"+"{{ next_ds_nodash }}",
    ...
    for c_tble in TBLS_NO_PARTITION:
        cust_tbl_tasks.append(
            BigQueryOperator(
                task_id=c_tble
                ,sql= '/'.join(['sql', ('.'.join([c_tble, 'sql']))])  #f"sql/{c_tble}.sql"
