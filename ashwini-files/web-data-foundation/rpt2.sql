--select * from `datascience-222717.dwh_web.fct_page_visit_daily`  --1251879/385107-526899
 
with pages as (
  select * --choose what columns from pages you need here
...
, seg_with_lead_data as (
  ...
  , sfdc_lead_data as (
    select  distinct --LoD - lead_or_contact_id, tactic, campaign_type,lead_or_contact_persona, createddate 
                     --there are few records that dont hold up to this LoD, but they will be classified as data issues and ignored
      lead_or_contact_id
      , tactic
      , campaign_type
