this has got the most accurate rpt table code
create or replace table `data-sandbox-123.Workspace_Ashwini.rpt_24mar` as


with pages as ( 
  select coalesce(session_id , 'Null ')  session_id    --LoD: session_id, visit_start_time , page_visit_id. --visitor_id, domain
          , visitor_id
          , url
          , url_cleaned
          , visit_start_time
 ...
 , seg_with_lead_data as  ( -- LoD: session_id, visit_start_time ,page_visit_id,lead_or_contact_id  : domain,visitor_id
  ...
  , sfdc_lead_data as ( --LoD: campaign_member_id
    select  distinct
      lead_or_contact_id 
      , campaign_member_id
      , tactic            --bring all these columns to our inquiry table
      , campaign_type
  ...
  , seg_pages_inquiry as (--LoD: domain,session_id, visit_start_time ,page_visit_id,lead_or_contact_id , campaign_member_id 
...
,  sfdc_inquiries_campaign_type as (        --LoD: campaign_member_id
  ...
  , inquiry_metrics as (  -- LoD:domain,session_id, visit_start_time ,page_visit_id,lead_or_contact_id , campaign_member_id_2 
    select t1.* except(session_id, visit_start_time ,page_visit_id,lead_or_contact_id  )
      , coalesce(t1.session_id, 'Null ')session_id
      , t1.visit_start_time
      , coalesce(t1.page_visit_id, 'Null ')     page_visit_id
...
select distinct coalesce(date(date_trunc(t1.visit_start_time, week)) , date(date_trunc(im.visit_start_time, week))) visit_start_time_week
